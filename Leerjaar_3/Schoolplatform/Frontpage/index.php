
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Document</title>
    <link rel="stylesheet" href="Frontpage.css">
</head>
<body>
    <div id="FontPage">
        
    <div id="Logo">
            <svg xmlns="http://www.w3.org/2000/svg" height="56px" viewBox="0 -960 960 960" width="56px" fill="#e8eaed"><path d="M440-278v-394q-41-24-87-36t-93-12q-36 0-71.5 7T120-692v396q35-12 69.5-18t70.5-6q47 0 91.5 10.5T440-278Zm40 118q-48-38-104-59t-116-21q-42 0-82.5 11T100-198q-21 11-40.5-1T40-234v-482q0-11 5.5-21T62-752q46-24 96-36t102-12q74 0 126 17t112 52q11 6 16.5 14t5.5 21v418q44-21 88.5-31.5T700-320q36 0 70.5 6t69.5 18v-481q15 5 29.5 11t28.5 14q11 5 16.5 15t5.5 21v482q0 23-19.5 35t-40.5 1q-37-20-77.5-31T700-240q-60 0-116 21t-104 59Zm140-240v-440l120-40v440l-120 40Zm-340-99Z"/></svg>
    </div>
        <h1>LearnSpace</h1>
        <p>Jouw persoonlijke leeromgeving</p>

        <div id="FontPageButton">
            
            <a href="signup (main one).php"> <button id="SignUp" >Sign up</button> </a> 
            <a href=""> <Button id="LogIn">Log in</Button> </a> 
        </div>
    </div>

    <div id="Secbuttons">
        <a href="youtube.com"> <button id="SignUp">Sign up</button></a>
        <a href=""> <button id="LogIn">Log in</button></a>
    </div>
</body>
</html>

